...
- **Anatomy:** Left grouped navigation rail; one center instrument viewport; right contextual panel and nested local preferences; one-row status rail. This follows the three-column shell in `The Athanor Design Restart.zip` while retaining responsive drawers below desktop width.
