...
use crate::protocol::{self, CommandIdentity, HostBinding, Inbound, PaperBoatReceiptSnapshot, ProjectionCursor, RecallPolicyProjection, ReceiptStatus};
...
impl IPanelContainer for AthanorHealth {
    fn init(base: Base<PanelContainer>) -> Self {
        Self {
            session_path: NodePath::default(), disclosure_label: NodePath::default(), transport_label: NodePath::default(),
            binding_label: NodePath::default(), recall_label: NodePath::default(), boat_label: NodePath::default(),
            refusal_label: NodePath::default(), detail_label: NodePath::default(), unavailable_label: NodePath::default(),
            refresh_button: NodePath::default(), session: None, binding: None, recall_cursor: None,
            recall_projection: None, boat: None, last_refusal: None, recall_request: None, boat_request: None,
            detail: "waiting for the shared Host session".into(), bound: None, base,
        }
    }
...
impl AthanorHealth {
    ...
    where T: GodotClass + Inherits<Node> {
        if path.is_empty() { missing.push(name.into()); return None; }
        match self.base().try_get_node_as::<T>(path) {
            Some(node) => Some(node),
            None => { missing.push(name.into()); None }
        }
    }
    ...
    fn resolve_bindings(&mut self) {
        ...
        let transport = self.resolve::<Label>(&self.transport_label, "transport_label", &mut missing);
        ...
        if !missing.is_empty() {
            godot_error!("AthanorHealth: missing scene bindings: {}", missing.join(", "));
            return;
        }
        self.bound = Some(Bound {
            disclosure: disclosure.expect("resolved"), transport: transport.expect("resolved"), binding: binding.expect("resolved"),
            recall: recall.expect("resolved"), boat: boat.expect("resolved"), refusal: refusal.expect("resolved"),
            detail: detail.expect("resolved"), reason: reason.expect("resolved"), refresh: refresh.expect("resolved"),
        });
    }
    ...
    fn disabled_reason(&self) -> Option<&'static str> {
        ...
        if !connected { Some("NO HOST CONNECTION") }
        else if self.session.as_ref().and_then(|session| session.bind().binding()).is_none() { Some("NO AUTHENTICATED HOST BINDING") }
        else if self.recall_request.is_some() || self.boat_request.is_some() { Some("SNAPSHOT REQUEST ALREADY PENDING") }
        else { None }
    }

    fn request_snapshots(&mut self) {
        if self.disabled_reason().is_some() { return; }
        ...
        let recall = protocol::resync_command(
            &CommandIdentity { message_id: recall_id.clone(), idempotency_key: recall_id.clone(), causation_id: String::new() },
            &binding,
        );
        ...
        let boat = protocol::paper_boat_receipt_subscribe_command(&CommandIdentity {
            message_id: boat_id.clone(), idempotency_key: boat_id.clone(), causation_id: String::new(),
        });
        match session.bind_mut().send(&boat) {
            Ok(()) => { self.boat_request = Some(boat_id); self.detail = "recall and Paper Boat snapshots requested through the shared Host session".into(); }
            Err(reason) => self.detail = format!("recall requested; Paper Boat snapshot not requested: {reason}"),
        }
    }
    ...
    fn render(&mut self) {
        ...
        bound.recall.set_text(&match (&self.recall_cursor, &self.recall_projection) {
            (Some(cursor), Some(projection)) => format!(
                "RECALL POLICY · VERSION {} · SEQUENCE {}\n{}",
                cursor.version, cursor.sequence,
                projection.degraded.as_ref().map(|reason| format!("◇ DEGRADED · {reason}")).unwrap_or_else(|| "◆ NO DEGRADATION REPORTED".into()),
            ),
            _ => format!("RECALL POLICY · VERSION {ABSENT} · SEQUENCE {ABSENT}\nDEGRADED {ABSENT}"),
        });
        bound.boat.set_text(&self.boat.as_ref().map(|snapshot| {
            let status = match snapshot.status { ReceiptStatus::Pending => "PENDING", ReceiptStatus::Delivered => "DELIVERED", ReceiptStatus::Degraded => "DEGRADED", ReceiptStatus::Refused => "REFUSED" };
            let integrity = snapshot.receipt.as_ref().map(|receipt| receipt.integrity_sha256.as_str()).unwrap_or(ABSENT);
            format!("PAPER BOAT · {status}\nINTEGRITY SHA256 {integrity}")
        }).unwrap_or_else(|| format!("PAPER BOAT · {ABSENT}\nINTEGRITY SHA256 {ABSENT}")));
        bound.refusal.set_text(&format!("LAST PROTOCOL REFUSAL\n{}", self.last_refusal.as_deref().unwrap_or(ABSENT)));
        bound.detail.set_text(&self.detail);
        bound.refresh.set_disabled(disabled_reason.is_some());
        let reason = disabled_reason.map(|reason| format!("ACTION UNAVAILABLE: {reason}")).unwrap_or_else(|| "ACTION AVAILABLE: REQUEST EXISTING HOST SNAPSHOTS".into());
    ...
    fn apply_recall(&mut self, envelope: &VarDictionary) {
        match protocol::parse_inbound(envelope) {
            ...
            Err(reason) => self.last_refusal = Some(format!("Recall Policy envelope refused: {reason}")),
        }
    }
    ...
    fn apply_boat(&mut self, envelope: &VarDictionary) {
        match protocol::parse_paper_boat_receipt(envelope) {
            Ok(snapshot) => {
                if let Ok(binding) = HostBinding::parse(envelope) { self.binding = Some(binding); }
                self.boat = Some(snapshot);
                self.detail = "authenticated Paper Boat receipt applied".into();
            }
            Err(reason) => self.last_refusal = Some(format!("Paper Boat envelope refused: {reason}")),
        }
    }
...
impl AthanorHealth {
    #[func]
    fn on_host_opened(&mut self) { self.detail = "Host transport opened; awaiting authenticated EventMeta".into(); self.render(); }

    #[func]
    fn on_host_closed(&mut self, detail: GString) {
        self.binding = None; self.recall_request = None; self.boat_request = None;
        self.detail = format!("Host disconnected: {detail}"); self.render();
    }

    #[func]
    fn on_host_malformed(&mut self, detail: GString) {
        self.last_refusal = Some(format!("malformed JSON: {detail}")); self.recall_request = None; self.boat_request = None;
        self.detail = "Host JSON refused".into(); self.render();
    }

    #[func]
    fn on_host_unavailable(&mut self, detail: GString) { self.detail = detail.to_string(); self.render(); }
    ...
    fn on_host_message(&mut self, envelope: VarDictionary) {
        ...
        if correlation.as_deref() == self.recall_request.as_deref() { self.recall_request = None; }
        if correlation.as_deref() == self.boat_request.as_deref() { self.boat_request = None; }
        self.render();
    }

    #[func]
    fn on_refresh_pressed(&mut self) { self.request_snapshots(); self.render(); }
}
