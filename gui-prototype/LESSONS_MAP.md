...
### Proof receipt — 2026-09-05 live status strip and lane trace drawer

- Architecture: the five hardcoded footer chips and the three hardcoded Account state rows became one live round owned by the new `health.js` — a single `/live/health` read behind the same module shape as `pulse.js` (idle → pending → live | failed), rendered in two places by `renderStatusStrip` and `renderAccountState`, so the footer can never say `Host ok` while the drawer says `Offline`. `serve.ts` grew a method per allow-list entry (`/live/health` is the Host's only GET read and is called without a body) and gained `/live/insula/trace`; the bearer stays server-side and the map is still the only reachable surface.
- Chromium at `1440 × 1000`, LIVE against the kodo Host on `:8787`: the strip read `Host ok · kodo`, `Recall seq 2,739`, `Delivery connected`, with steady dots on host and delivery. Popovers disclosed the Host's own words — schema version 1 and WebSocket path `/room/kodo/athanor/v1/ws`; projection `recall_policy` at version/sequence `2,739` and state hash `2195a1fa3646…`, named explicitly as the recall policy cursor and **not** a count of memories; `AKASHA enabled · broker configured · status connected · latest receipt event ad222f60-…-12dc7fceb375 · stream sequence 174`. The Account block read `Surface = Live reads · writes none`, `Host = ok · kodo room`, `Persistence = PostgreSQL ok`, and spoke the insula reading in full (`ok after 604 successful and 0 failed operations, at API schema 1`). `Query Host` inside the popover re-queried through `Host querying…` back to `Host ok · kodo`.
- Truthful absence, not invented values: `body` and `kittens` render `Body not reported by Host` / `Kittens not reported by Host` and name why — no embodied-session or kitten field exists in the round — and enumerate the nine fields the Host does answer. They hold that state through a failed read too, because the absence is in the health contract, not in the round. Nothing was upgraded: `Persistence` speaks only the Host's insula status word, since the insula pool is the round's only persistence reading.
- Loss now distinguishes zero from unknown: LIVE read `0 dropped · No writer drops · drop_count_sum summed to zero across 657 rollup rows`; with no rollup rows in window the channel states `Not known · No rollup rows in window · nothing to sum, so drops are unknown, not zero`. The prior wording asserted `0 dropped · writer-side drop receipts` without saying whether zero was measured.
- Trace drilldown, all five states rendered: clicking a lane opens one drawer. A lane whose source carries no trace identity states it exactly — `insula.vitals.minute v1 rollups aggregate many spans and carry no trace id. The Host's trace route reads one exact trace id, and it serves no route that lists a lane's spans` — which is this surface's resting state today and the honest boundary, since `query_trace` filters `WHERE trace_id=$2::uuid` and no Host route enumerates traces. With an identity supplied at the page boundary, the drawer read **real Host-served rows**: `insula.trace v1 · trace c185225a-8f19-4370-9ec7-25e8caccbfc9`, 6 spans across `provider_request` (20.3 s), `tool_call` (190.5 ms), `provider_usage`, `tool_result`, with parent span ids and per-span outcome, error-class, and drop flags. A canonical-but-unknown trace rendered `No spans · answered zero rows for trace 00000000-… inside this room's scope`; a non-canonical id rendered the Host's own `Trace refused · Host refused: invalid_request`; hostile markup as a trace id produced zero injected nodes and no script execution.
- Failure proof, with the live House untouched: transport failure injected at the page's fetch boundary (never by stopping `athanor.exe`) produced `Host unreachable`, `Recall unreachable`, `Delivery unreachable` with attention dots, `Surface = Local only · Host unreachable`, and the source line `Host unreachable · Failed to fetch`; restoring transport and re-querying returned every channel to live. A well-formed 200 carrying a non-health body was refused as `The health read failed: Host answered without a health status` rather than rendered as a value.
- Two bugs found on the rendered surface and fixed: clicking the popover's `Query Host` detached the button mid-dispatch, so the document-level outside-click handler saw a target outside the strip and closed the popover — now a target this click already removed is ignored, and the open popover re-renders with each round instead of going stale behind an updated chip. Opening a lane drawer also lost keyboard focus to the panel re-render; `renderWithLaneFocus` hands it back across a double frame, verified by four rapid cycles on one lane ending closed with focus still on `omp_adapter tool_call`.
- Chromium at `390 × 844`: compact chips replaced the full text (`Host ok`, `Recall 2,739`, `No body`, `No count`, `Connected`), the strip ended at `386 px` without scrolling, the Account block ended at `275 px` with its insula note wrapping, and the trace drawer occupied `34–356 px` with spans collapsed to one `292 px` column, widest inner element at `341 px`. Channels collapsed to one `334 px` column. Zero horizontal document overflow and zero elements past the right edge at both viewports.
- Safety: zero page errors at both viewports. The bearer token and `runtime-secrets` appear in none of the 11 served sources checked. `/live/insula/events` — a write route — is refused `404` by the allow-list, and a `GET /live/health` from the page is refused `405`; only the proxy converts the page's POST into the Host's GET.
- Named residuals, not this wave's: the standalone-serve font 404 (`/site/fonts/InterVariable.woff2`, `c7ad11f`, Pages-owned) remains the only unexpected console 404. The mechanics observatory's category row still puts two elements past the right edge at `1440 px` (`Advanced Guardrails` button to `1457 px`); measured identically on the unmodified prototype, so it is pre-existing and causes no document overflow. The receipts block stays snapshot-only and its chip still says `no Host route yet`, because no Host route serves receipt kinds. The trace drawer's live branch cannot be reached by the operator until a Host route publishes a lane's trace identity; adding one needs a new AKASHA query and was outside this packet's Rust boundary.
- Open authority gate: the implementation and rendered evidence are complete; whether the strip's new wording earns its footer space remains Sol's own squint-test verdict.

### Proof receipt — 2026-09-05 lane spans: the operator-reachable trace id

- Closes the BUGS.md row `Pulse trace drawer has no operator-reachable trace id`. The drawer's live branch was proven last wave but unreachable, because a lane derived from `insula.vitals_minute` rollups carries no trace id and no Host route listed spans. The lane drawer is now two stages: clicking a lane asks the new `/live/insula/spans` for that lane's newest settled spans, and clicking one of those opens the existing trace drawer for its `trace_id`.
- Architecture: one bounded AKASHA read, `insula.spans.recent v1` (`query_spans` in `crates/akasha/src/insula/query.rs`), behind one new Host route `/athanor/v1/insula/spans` sharing the family's `require_bearer` middleware, exposed to the page as the POST allow-list entry `/live/insula/spans`. `window` is a closed set (`15m` | `1h` | `24h`) rather than a caller-supplied interval, because only a recent lower bound keeps the read an index walk. `room` is required with no wildcard — unlike `TraceScope::room` — since every returned `span_id`/`trace_id` is a working key into `insula/trace`, so a house-wide lane read would let one room enumerate another's traces. House and room both come from the Host's trusted binding; the body names only the lane.
- Index, not a scan: migration 0022 left nothing answering this shape. `idx_insula_log_vitals` leads on `observed_at`, so house/room/operation are in-index filters rather than a search prefix, and `idx_insula_log_window` is that walk with no filtering at all — a multi-million-row walk to return ten. Added the next numbered idempotent migration `0029_insula_log_lane_spans.sql`: `(house_id, room, operation, observed_at DESC, span_id DESC)`. EXPLAIN on the isolated schema, against the shipped `SPANS_SQL` const itself so the proof cannot drift from the statement: `Limit → Index Scan using idx_insula_log_lane_spans on log`, `Index Cond` carrying all three equality columns plus `observed_at >= now() - interval`, `phase` as a recheck filter, and **no Sort node**. `phase`/`outcome_class` are deliberately absent from the index: placing them ahead of `observed_at` would break the ordering prefix for the unfiltered lane read the drawer performs on every click.
- Two real defects the SQL and the surface each gave up only under proof. First, `ORDER BY span_id DESC` silently bound to the `span_id::text` **output alias** instead of the uuid column, so the plan showed `Sort Key: ((span_id)::text) DESC` and the index could never serve it; the ordering is now qualified `log.observed_at, log.span_id`. Second, activating a span by keyboard handed focus back to the lane `<summary>`, stranding the operator above all eight rows with the trace drawer they had just opened below — `renderWithSpanFocus` now returns focus to the pressed span row, verified as `BUTTON {"span":"fde9e038-…"}` where it had read `SUMMARY {"lane":"omp_adapter tool_call"}`.
- Chromium at `1440 × 1000` and `390 × 844`, LIVE against a real Insula router reading real PostgreSQL rows. All four surface states rendered at both viewports: the spans list (`8 spans`, `insula.spans.recent v1 · last 24h · queried 12:46 local · truncated at 8`, eight `<button>` rows each carrying observed time, span id, trace id, duration and outcome, with `error`/`tool_error` flags where the row carried them); the drill-down (clicking the newest row opened `Trace · 2 spans · insula.trace v1 · trace 1b2129bc-abd7-4b8e-bf9e-f60ac1c3f1ea`, whose `tool_call` row is span `31c240df-c2c0-4062-921d-e7e49a1cf001` — the exact span pressed — beside its child `provider_request`, satisfying the row's own acceptance: the same spans `insula/trace` returns for that id); the empty window (`No spans in window`, `insula.spans.recent v1 answered zero settled spans for knock_claim in the last 24h inside this room's scope`); and the refusal (`Spans refused`, `Host refused: invalid_request`, `Requested lane Bad Lane.`) driven by a genuine Host `422`, not an injected one — a seeded rollup whose operation is not a mechanical atom, refused by `query_spans` before it reached the pool. `aria-pressed` marks exactly the chosen row (`0:false 1:false 2:true …`). Zero horizontal document overflow and zero elements past the right edge at both viewports; zero JavaScript exceptions.
- Proof-plane honesty, and the live half owed: the installed Host on `:8787` does not carry this route, so it could not answer it. The rendered proof ran against a throwaway harness serving the **real** `InsulaHost` router — same handler, same bearer middleware, same `query_spans`, real rows in the isolated `athanor_substrate_test` Insula schema — on `:8799`, with a second prototype server on `:4187` pointed at it by the new `PULSE_HOST_PORT` override in `serve.ts`. The live `athanor.exe`, the live `:8787` Host, and `:4186` were never touched or stopped. The harness has been deleted; the BUGS row therefore moves to **Repaired but not deployed**, with the deployed-Host half explicitly owed rather than claimed.
- Named residuals, not this wave's: the standalone-serve font 404 (`/site/fonts/InterVariable.woff2`, Pages-owned) still appears. `/live/health`, `/live/docket/board` and `/live/hallway/inbox` returned 404 during this proof because the harness serves the Insula family only — the footer honestly read `Host unreachable` while the Pulse panel read `Host connected`, which is the separate-channels rule working, not a regression. Deployed against a full Host both go live together.
- Open authority gate: implementation and rendered evidence are complete. Whether a lane should default to `24h` rather than a shorter window, and whether eight rows is the right handful, remain Sol's own squint-test verdicts.

### Proof receipt — 2026-09-05 mechanics category overflow

- Repair: `.mechanics-categories` now wraps at all widths. The old desktop horizontal scroll rule is removed. The mobile override now applies only to `.shelf-marks`. Chip spacing, borders, type, count badges, and markup are unchanged.
- Browser proof: served the development checkout with `PULSE_PORT=4186 bun gui-prototype/serve.ts`. Opened headless Chrome with `--remote-debugging-port=9346` and a separate proof profile. Attached through CDP, used `page.setViewport`, opened House slot 2, and measured `getBoundingClientRect().right`. Waited for resize animations to finish before the mobile measurement.
- Right edges in pixels, in category order: All; Recall & Context; Memory, Lessons & Anamnesis; GIGA & Embeddings; Host, Delivery & Hallways; Rooms & Sessions; Backups; Advanced Guardrails.
- At `1440 × 1000`, button right edges were `408.4375, 541.03125, 743.9375, 896.140625, 1072.265625, 1213.328125, 1305.140625, 496.078125`. Count right edges were `398.4375, 531.03125, 733.9375, 886.140625, 1062.265625, 1203.328125, 1295.140625, 486.078125`. All eight buttons fit inside the column's `1360 px` right edge. The last chip wraps to the next line.
- At `390 × 844`, button right edges were `71.4375, 204.03125, 209.90625, 362.109375, 183.125, 324.1875, 98.8125, 250.890625`. Count right edges were `61.4375, 194.03125, 199.90625, 352.109375, 173.125, 314.1875, 88.8125, 240.890625`. All eight buttons fit inside the column's `367 px` right edge.
- At both viewports, all eight count rectangles fit fully inside their buttons. The settled page had zero positive-size elements past the viewport's right edge and zero horizontal document overflow. Screenshots confirmed visible chips and counts. At the mobile viewport, clicking Advanced Guardrails set its `aria-pressed` value to `true`.
- Proof boundary: this checks the development browser surface and its existing source-census categories. No deployment, durable write, or live process stop was performed. The deployed surface still needs the same two-viewport check after deployment.
