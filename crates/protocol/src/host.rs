use hearth::context::{ContextAnalysis, ContextAnalysisRequest};
use hearth::conversation::VisibleMessage;
...
pub struct HallwayKnockSettlePayload {
    pub knock_id: String,
    pub outcome: hearth::hallway::HallwayKnockOutcome,
    #[serde(default)]
...
/// `akasha::LessonFamily` on the wire; the protocol crate cannot
...
/// freshness, dedupe, and transcript shape lives in `hearth::conversation`.
#[derive(Clone, Debug, Default, Deserialize, Eq, PartialEq, Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
...
pub struct HallwayKnockClaimedEvent {
    #[serde(flatten)]
    pub meta: EventMeta,
    pub result: hearth::hallway::HallwayKnockClaimReceipt,
}
...
pub struct HallwayKnockSettledEvent {
    #[serde(flatten)]
    pub meta: EventMeta,
    pub result: hearth::hallway::HallwayKnockSettleReceipt,
}
