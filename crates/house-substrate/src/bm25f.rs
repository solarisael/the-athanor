...
mod tests {
    ...
    fn frequencies(values: &[(&str, u64)]) -> BTreeMap<String, u64> {
        values
            .iter()
    ...
    #[test]
    fn title_match_outweighs_one_body_match() {
        let query = vec!["athanor".to_string()];
        let df = frequencies(&[("athanor", 2)]);
        let title = score(
            &query,
            100,
            &df,
        ...
        assert!(title.value > body.value);
        assert_eq!(title.matched_fields, vec!["title"]);
    }

    #[test]
    fn repeated_terms_saturate_instead_of_growing_linearly() {
        let query = vec!["retrieval".to_string()];
        let df = frequencies(&[("retrieval", 5)]);
        let once = score(
            &query,
            100,
            &df,
    ...
    #[test]
    fn field_length_normalization_penalizes_long_fields() {
        let query = vec!["needle".to_string()];
        let df = frequencies(&[("needle", 2)]);
        let short = score(
            &query,
            100,
            &df,
        ...
        let long_text = format!("needle {}", "hay ".repeat(99));
        let long = score(
            &query,
            100,
            &df,
