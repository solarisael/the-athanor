...
mod tests {
    ...
    #[test]
    fn creates_standalone_lineage_memories() {
        let memories = normalize_quest_memories(QuestBatch {
            tool_call_id: Some("call-7".into()),
            tasks: vec![QuestTask {
                name: Some("Quill".into()),
                agent: Some("scout".into()),
                task: Some(
                    "# Target\ncrates/akasha/src/remember.rs\n\n# Change\nRepair it."
                        .into(),
                ),
            }],
            results: vec![SettledQuest {
                ...
                id: Some("Quill".into()),
                agent: Some("scout".into()),
                output: Some("Thread keys survive writes.".into()),
                exit_code: Some(0),
                ..SettledQuest::default()
            }],
        });

        assert_eq!(memories.len(), 1);
        assert_eq!(memories[0].idempotency_key, "call-7:Quill");
        assert_eq!(
            memories[0].threads,
            [
                "kitten:quill".to_string(),
                "domain:crates-akasha-src-remember-rs".to_string(),
            ]
        );
        assert!(
            memories[0]
                .body
                .contains("Outcome: completed; role: scout.")
        );
    }

    #[test]
    fn ignores_in_flight_results_and_preserves_terminal_errors() {
        let in_flight = normalize_quest_memories(QuestBatch {
            tool_call_id: None,
            tasks: vec![QuestTask {
                task: Some("# Target\nLesson retrieval".into()),
                ..QuestTask::default()
            }],
            results: vec![SettledQuest::default()],
        });
        assert!(in_flight.is_empty());

        let failed = normalize_quest_memories(QuestBatch {
            tool_call_id: None,
            tasks: vec![QuestTask {
                task: Some("# Target\nLesson retrieval".into()),
                ..QuestTask::default()
        ...
        assert!(failed[0].body.contains("Report\ndatabase unavailable"));
        assert!(failed[0].body.contains("Outcome: failed."));
    }

    #[test]
    fn settles_lineage_only_for_terminal_lifecycles() {
        let running = normalize_lifecycle_memory(
            QuestLifecycle {
                status: Some("running".into()),
                task: Some("# Target\nsrc/routing.ts".into()),
                ..QuestLifecycle::default()
            },
            "partial",
        );
        assert!(running.is_none());

        let aborted = normalize_lifecycle_memory(
            QuestLifecycle {
                ...
                session_file: Some("C:/sessions/Quill.jsonl".into()),
            },
            "   ",
        )
        .expect("an aborted quest still settles");
        assert_eq!(aborted.idempotency_key, "call-3:Quill");
        assert!(aborted.body.contains("Report\nQuest aborted."));
        assert!(aborted.body.contains("Outcome: aborted; role: scout."));
    }

    #[test]
    fn resolves_the_report_beside_the_child_session() {
        assert_eq!(
            quest_report_path("C:/sessions/Quill.JSONL").as_deref(),
            Some("C:/sessions/Quill.md")
        );
        assert_eq!(
            quest_report_path("C:/sessions/Quill").as_deref(),
            Some("C:/sessions/Quill.md")
        );
        assert_eq!(quest_report_path("   "), None);
    }

    #[test]
    fn derives_domain_from_windows_and_relative_targets() {
        assert_eq!(
            quest_domain(
                "# Target\nC:\\work\\the-athanor\\crates\\akasha\\src\\lesson.rs\n# Change\nRepair it"
            ),
            "src-lesson-rs"
        );
        assert_eq!(
