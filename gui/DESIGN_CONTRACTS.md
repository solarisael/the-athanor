...
- **Anatomy:** Thin top chrome; left nested navigation reliquary; one center instrument viewport; right context/settings reliquary; one-row status rail. Four-corner ornament, ritual seams, and phase-local gold identify interactive boundaries without claiming authority.
- **Variants:** Wide at 1200 px and above preserves the desktop four-region reading order with both reliquaries docked. Compact from 800–1199 px keeps navigation docked and opens context/settings over the center with a scrim. Narrow below 800 px keeps the center primary and opens either reliquary as an exclusive drawer.
- **States:** Active screen, responsive class, optional local layout override, open drawer, and an independent pane stack for each reliquary. These are local presentation states owned by `AthanorProbe` and `AthanorReliquaryNavigator`, never Host state.
...
- **Composition:** The active conversation owns the center. The right column holds route, authority, selection, and local settings context instead of becoming a generic placeholder. The bottom rail exposes only Host binding and compact identity; detail belongs in the right reliquary.
- **Web mapping:** The website supplies content-first hierarchy, localized gold, functional ornament, concentric reading, four-corner reliquary geometry, ritual separators, and nested side-menu grammar. Its WebGPU/WASM renderer is not the Athanor runtime; this client remains Godot.
- **Godot mapping:** `navigation/reliquary_navigator.gd` is reusable pane-stack technology driven by `pane_target` and `action_id` metadata. `AthanorProbe` owns screen routing, viewport classes, center margins, exclusive drawers, local layout overrides, and Escape delegation. `main.tscn` contains the two navigator instances and no flat global screen strip.
- **Proof:** `navigation/tests/reliquary_contract.gd` proves nested panes, hidden inactive panes, focus restoration, Escape grammar, wide four-region geometry, compact context overlay, narrow exclusive drawers, and the 42 px status rail. Native Forward+ frames were inspected at 1440×900, 1000×760, and 640×760, including wide and narrow settings surfaces.
- **Open gaps:** The exact fourteen-screen friend map is not recovered and is explicitly not fabricated. Conversation, source, GIGA, lineage, familiar-index, and selection-detail projections remain open Host contracts. Motion is not authored, so reduced-motion control remains absent rather than decorative.
