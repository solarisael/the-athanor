...
- **First cut:** make `recall.content` use the trigram index (`$1 <% c.body` under `SET LOCAL pg_trgm.word_similarity_threshold`, per-term indexable ILIKE instead of `ILIKE ANY`, the `$5 = '{}'` guard branched in Rust), then compute `word_similarity` only on survivors. Expected 1.3 s → tens of ms. **Second cut:** the embed client timeout 20 s → about 3 s for recall (degrade to lexical), which removes the 21 s tail.
