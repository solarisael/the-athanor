...
impl HostLink {
    ...
    pub fn validate_url(url: &str) -> Result<(), String> {
        ...
        if !trimmed.starts_with("ws://") && !trimmed.starts_with("wss://") {
            return Err("Host address must begin with ws:// or wss://".to_string());
        }
        if trimmed.len() <= "wss://".len() {
            return Err("Host address has no host or port".to_string());
        }
        Ok(())
    }
    ...
    pub fn open(&mut self, url: &str, bearer_token: &str) -> Result<(), String> {
        ...
        if bearer_token.is_empty() {
            return Err(
                "ATHANOR_HOST_TOKEN is empty; the client will not connect without authentication"
                    .to_string(),
            );
        }
        ...
        if error != GodotError::OK {
            return Err(format!("Godot refused the connection: {error:?}"));
        }
        self.url = trimmed;
    ...
    pub fn send(&mut self, envelope: &VarDictionary) -> Result<(), String> {
        if self.phase != LinkPhase::Open {
            return Err("the Host connection is not open".to_string());
        }
        let text = Json::stringify(&envelope.to_variant());
        let error = self.peer.send_text(&text);
        if error != GodotError::OK {
            return Err(format!("Godot refused the send: {error:?}"));
        }
        Ok(())
    }
        ...
        while self.peer.get_available_packet_count() > 0 {
            ...
            if text.to_string().is_empty() {
                events.push(LinkEvent::Malformed {
                    detail: "empty or non-UTF-8 packet".to_string(),
                });
                continue;
            }
            ...
            match parsed.try_to::<VarDictionary>() {
                ...
                Err(_) => events.push(LinkEvent::Malformed {
                    detail: "packet is not a JSON envelope".to_string(),
                }),
            }
        }
