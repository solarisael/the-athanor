...
use godot::classes::{Button, IPanelContainer, Label, LineEdit, Node, OptionButton, PanelContainer, TextEdit};
...
impl AthanorDispatch {
    ...
    fn resolve_bindings(&mut self) {
        ...
        let familiar = self.resolve::<LineEdit>(&self.familiar_field, "familiar_field", &mut missing);
        ...
        let acceptance = self.resolve::<TextEdit>(&self.acceptance_field, "acceptance_field", &mut missing);
        ...
        let diagnostics = self.resolve::<Label>(&self.diagnostics_label, "diagnostics_label", &mut missing);
        let dispatcher = self.resolve::<Label>(&self.dispatcher_label, "dispatcher_label", &mut missing);
        ...
        if !missing.is_empty() {
            godot_error!("AthanorDispatch: missing scene bindings: {}", missing.join(", "));
            return;
        }
        self.bound = Some(Bound {
            disclosure: disclosure.expect("resolved"), lane: lane.expect("resolved"), familiar: familiar.expect("resolved"),
            task: task.expect("resolved"), target: target.expect("resolved"), acceptance: acceptance.expect("resolved"),
            risk: risk.expect("resolved"), send: send.expect("resolved"), state: state.expect("resolved"),
            receipt: receipt.expect("resolved"), diagnostics: diagnostics.expect("resolved"), dispatcher: dispatcher.expect("resolved"),
            packet: packet.expect("resolved"), detail: detail.expect("resolved"), reason: reason.expect("resolved"),
        });
    }
    ...
    fn wire_session(&mut self) {
        ...
        for (signal, method) in [
            ("opened", "on_host_opened"), ("closed", "on_host_closed"),
            ("malformed", "on_host_malformed"), ("unavailable", "on_host_unavailable"),
    ...
    fn wire_controls(&mut self) {
        ...
        bound.task.connect("text_changed", &Callable::from_object_method(&this, "on_form_changed"));
        bound.send.connect("pressed", &Callable::from_object_method(&this, "on_send_pressed"));
    }
    ...
    fn disabled_reason(&self) -> Option<&'static str> {
        ...
        if !connected {
            ...
            .bound
            .as_ref()
            .map_or(true, |bound| bound.task.get_text().to_string().trim().is_empty())
        {
    ...
    fn send_request(&mut self) {
        ...
        let acceptance = bound.acceptance.get_text().to_string().lines().map(str::trim).filter(|line| !line.is_empty()).map(str::to_owned).collect::<Vec<_>>();
        let risk = match bound.risk.get_selected() { 0 => "low", 1 => "medium", 2 => "high", _ => { self.detail = "dispatch refused locally: unknown risk selection".into(); self.render(); return; } };
        let Some(session) = self.session.as_mut() else { return; };
        let Some(binding) = session.bind().binding() else { return; };
        let message_id = session.bind_mut().new_identifier();
        let command = protocol::routing_dispatch_command(
            &binding,
            &CommandIdentity { message_id: message_id.clone(), idempotency_key: message_id.clone(), causation_id: String::new() },
            &lane, &familiar, &task,
    ...
    fn packet_text(packet: Option<&SpawnPacketView>) -> String {
        let Some(packet) = packet else { return ABSENT.into(); };
        let tasks = packet.tasks.iter().enumerate().map(|(index, task)| {
            format!("TASK {}\nname: {}\nagent: {}\ntask:\n{}", index + 1, task.name, task.agent.as_deref().unwrap_or(ABSENT), task.task)
        }).collect::<Vec<_>>().join("\n\n");
    ...
    fn render(&mut self) {
        ...
        bound.state.set_text(if connected {
            if self.pending_correlation.is_some() { "◇ HOST CONNECTED · PACKET PENDING" } else { "◆ HOST CONNECTED · BUILDER READY" }
        } else { "◇ HOST UNAVAILABLE" });
        if let Some(projection) = &self.projection {
            bound.receipt.set_text(&format!(
                "{} {} · LANE {} · MODEL ROLE {} · OMP AGENT {} · EVENT {} · SEQ {}",
                if projection.ok { "◆" } else { "✕" }, projection.status.to_uppercase(),
                projection.lane.as_deref().unwrap_or(ABSENT), projection.model_role.as_deref().unwrap_or(ABSENT),
                projection.omp_agent.as_deref().unwrap_or(ABSENT), projection.event_id, projection.sequence,
            ));
            bound.diagnostics.set_text(&format!(
                "ERRORS\n{}\n\nWARNINGS\n{}",
                if projection.errors.is_empty() { ABSENT.into() } else { projection.errors.join("\n") },
                if projection.warnings.is_empty() { ABSENT.into() } else { projection.warnings.join("\n") },
            ));
            bound.dispatcher.set_text(&format!(
                "DISPATCHER EXECUTED: {} · REASON: {}",
                if projection.dispatcher_executed { "yes" } else { "no" }, projection.dispatcher_reason,
            ));
            bound.packet.set_text(&Self::packet_text(projection.spawn_packet.as_ref()));
        } else {
            bound.receipt.set_text(&format!("STATUS {ABSENT} · LANE {ABSENT} · MODEL ROLE {ABSENT} · OMP AGENT {ABSENT}"));
            bound.diagnostics.set_text(&format!("ERRORS\n{ABSENT}\n\nWARNINGS\n{ABSENT}"));
            bound.dispatcher.set_text(&format!("DISPATCHER EXECUTED: {ABSENT} · REASON: {ABSENT}"));
            bound.packet.set_text(ABSENT);
        }
        bound.detail.set_text(&self.detail);
        bound.send.set_disabled(disabled_reason.is_some());
        let reason = disabled_reason.map(|reason| format!("ACTION UNAVAILABLE: {reason}")).unwrap_or_else(|| "ACTION AVAILABLE: ASK HOST TO BUILD PACKET".into());
...
impl AthanorDispatch {
    #[func]
    fn on_host_opened(&mut self) { self.detail = "Host connected; waiting for authenticated binding".into(); self.render(); }

    #[func]
    fn on_host_closed(&mut self, detail: GString) { self.pending_correlation = None; self.detail = format!("Host disconnected: {detail}"); self.render(); }

    #[func]
    fn on_host_malformed(&mut self, detail: GString) { self.pending_correlation = None; self.detail = format!("malformed JSON refused: {detail}"); self.render(); }

    #[func]
    fn on_host_unavailable(&mut self, detail: GString) { self.pending_correlation = None; self.detail = detail.to_string(); self.render(); }
    ...
    fn on_host_message(&mut self, envelope: VarDictionary) {
        ...
        if correlation.as_deref() == self.pending_correlation.as_deref() {
            match protocol::parse_dispatch_result(&envelope) {
                Ok(projection) => { self.pending_correlation = None; self.detail = "authenticated packet receipt applied; nothing was executed".into(); self.projection = Some(projection); }
                Err(reason) => { self.pending_correlation = None; self.detail = format!("dispatch receipt refused: {reason}"); }
            }
        }
        self.render();
    }
    ...
    #[func]
    fn on_form_changed(&mut self) { self.render(); }

    #[func]
    fn on_send_pressed(&mut self) { self.send_request(); self.render(); }
}
