...
//!
//! The vocabulary (scope tokens, urgency, cooldown, the spec's shape) lives in
//! `hearth::lesson_triggers`. This module owns the compilers: whether a
//! pattern parses, which grammar a surface path speaks, whether a pattern
//! fires on a surface, and the per-room compile cache.
//!
//! Nothing here touches the database or the clock. The caller reads rows,
...
use hearth::lesson_triggers::{
    Cooldown, LessonTriggerSpec, PatternKind, ScopeToken, SurfaceKind, Urgency,
};
use regex::Regex;
...
// feature in crates/akasha/Cargo.toml and fill in the grammar, or add a
...
/// Write-time refusal for patterns that can never fire: a regex that does not
/// compile, or an ast pattern no supported grammar parses. The spec's shape
/// is the vocabulary's job; call this after `validate_fields`.
pub fn validate_patterns(spec: &LessonTriggerSpec) -> Result<(), String> {
    for pattern in &spec.condition {
    ...
    for pattern in &spec.ast_condition {
...
fn ast_pattern_parses(pattern: &str) -> bool {
    EXTENSION_LANGUAGES
        .iter()
...
fn language_slug(path: Option<&str>) -> Option<&'static str> {
...
fn ast_language(path: Option<&str>) -> Result<SupportLang, String> {
...
impl CompiledTrigger {
    fn admits(&self, surface: &Surface<'_>) -> bool {
        ...
        self.scopes
            .iter()
            .any(|scope| scope.admits(surface.kind, surface.tool))
    }
...
mod tests {
    ...
    #[test]
    fn write_validation_refuses_patterns_that_can_never_fire() {
        let good = LessonTriggerSpec {
            condition: vec!["\\bunwrap\\(\\)".into()],
            ast_condition: vec!["$A.unwrap()".into()],
            ..Default::default()
        };
        assert_eq!(validate_patterns(&good), Ok(()));
